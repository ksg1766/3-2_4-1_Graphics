...
intensity(const Ray& ray, const Light l[], int nLights, int depth, int E = -1)
{
	...
	if (iObject != -1) // Hit an object
	{
		...
		m_diffuse = _m_diffuse[iObject];
		//if(pow(((eye - center_world[iObject]).x), 2) + pow(((eye - center_world[iObject]).y), 2) + pow(((eye - center_world[iObject]).z), 2) < pow(radius[iObject], 2))	// In the world coordinate system
		//float	radius[8];
		if (iObject == 7)
			n = -n;
		...
		for (int i = 0; i < nLights; i++)
		{
			...
			vec3	pDistantLight = p + 0.3e1f * l[i].p_eye;
		...
		if (depth < DEPTH)
		{
			...
			vec3	pr = p + 0.3E1f * r;	// Point far awary from p along r
...
rayTracing()
{
	...
	// Modeling matrix
	{
		...
		// viewModel = viewMode1 * rotation matrix
...
#pragma omp master
	...
	center_world[nSpheres] = vec3(0, 0, -r);	radius[nSpheres] = 0.5f;	nSpheres++;
	center_world[nSpheres] = vec3(0, 0, 0);		radius[nSpheres] = 8.00f;	nSpheres++;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (rayTracingRequired)
		{
			...
			if (textureMapping) // Employ texture mapping to display the ray-traced image
			{
				...
				// Direct draw using glDrawPixe1s()
