...
// Light configuration
Vector4f	light(0.0f, 0.0f, 5.0f, 1); // Light position
...
const float MAX_ANGLE_CHANGE = float(M_PI) / 4;	// Max dtheta per IK ieration
...
//	Vector3f	ee_current = T.block<3,3>(0,0) * Vector3f(0, 0, 0) + T.block<3,1>(0,3);
	...
	switch (method)
	{
	case JACOBIAN_TRANSPOSE:	// The Jacobian transpose method
		{
...
mouse(GLFWwindow* window, int button, int action, int mods)
{
	if (action == GLFW_PRESS && button == GLFW_MOUSE_BUTTON_LEFT)
	{
		...
		//cout << "Mouse at (" MM xpos << ", " << ypos << ")" << endl;
