...
uniform vec3	LightPosition0 = vec3(-1, -1, -1);					// In the view coordinate system
uniform vec3	LightPosition1 = vec3(-1, -1, -1);					// In the view coordinate system
...
struct Light
{
	...
	vec3 diffuse;
	vec3 specular;
	vec3 attenuation;	// constant, linear, quardratic;
};
...
{
	// Lighting
	if(L0.position != vec3(-1, -1, -1))
		outColor = phongReflectionModel(L0, position, normal);
	if(L1.position != vec3(-1, -1, -1))
