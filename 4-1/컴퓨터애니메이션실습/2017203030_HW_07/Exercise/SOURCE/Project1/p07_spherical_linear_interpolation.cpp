...
void renderEx(GLFWwindow* window);
...
void update(float elapsed)
{
	...
	for (int i = 0; i < 10; i++)
	{
		_p = (1 - _t * i) * p1 + _t * i * p2;
		T1[i].block<3, 1>(0, 3) = _p;
		T2[i].block<3, 1>(0, 3) = _p;
	}
	...
	if (method == 7)
	{
		for (int i = 0; i < 10; i++)
		{
			T1[i].block<3, 3>(0, 0) = rlerp(_t * i, q1, q2);

			Quaternionf q;
			q = q1.slerp(_t * i, q2);
			T2[i].block<3, 3>(0, 0) = Matrix3f(q);
		}
	}
}
void drawMesh()
{
	...
	GLfloat mat_diffuse[4] = { 0.95f, 0.0f, 0.0f, 1 };
...
void drawWireFrame()
{
	...
	//GLfloat mat_ambient[4] = { 0.10f, 0.10f, 0.10f, 1 };
	GLfloat mat_diffuse[4] = { 0.0f, 0.95f, 0.0f, 1 };
	//GLfloat mat_specular[4] = { 0.10f, 0.10f, 0.10f, 1 };
	//GLfloat mat_shininess = 128;
...
void renderEx(GLFWwindow* window)
{
	...
	for (int i = 0; i < 10; i++)
	{
		glMultMatrixf(T1[i].data());
		drawMesh();
		Matrix4f _T = T1[i].inverse();
		glMultMatrixf(_T.data());

		glMultMatrixf(T2[i].data());
		drawWireFrame();
		_T = T2[i].inverse();
		glMultMatrixf(_T.data());
	}
