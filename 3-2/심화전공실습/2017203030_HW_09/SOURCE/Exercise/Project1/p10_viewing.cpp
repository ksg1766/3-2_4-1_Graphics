...
struct Camera {
	...
	vec3	f;	  //Bird-eye view
	vec3	r;
}	C;


// Light configuration			
vec4	light(0.0, 0.0, 0.8, 0);	// Initial light position
...
render(GLFWwindow* window)
{
	...
	C.a.x = C.e.x + cos(theta * M_PI / 180) * sqrt((C.a.x - C.e.x) * (C.a.x - C.e.x) + (C.a.y - C.e.y) * (C.a.y - C.e.y) + (C.a.z - C.e.z) * (C.a.z - C.e.z));
	C.a.z = C.e.z + sin(theta * M_PI / 180) * sqrt((C.a.x - C.e.x) * (C.a.x - C.e.x) + (C.a.y - C.e.y) * (C.a.y - C.e.y) + (C.a.z - C.e.z) * (C.a.z - C.e.z));
	...
	switch (selection)
	{
	case 0: drawSphere(0.7f, 64, 64);	break;
...
keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if ((action == GLFW_PRESS || action == GLFW_REPEAT) && !(mods & GLFW_MOD_SHIFT))
	{
		...
		case GLFW_KEY_LEFT:	 C.e -= 0.1f * C.f;	C.a -= 0.1f * C.f; break;
		case GLFW_KEY_RIGHT: C.e += 0.1f * C.f;	C.a += 0.1f * C.f; break;
		}
	}
	...
	if ((action == GLFW_PRESS || action == GLFW_REPEAT) && (mods & GLFW_MOD_SHIFT))
	{
		...
		{
			// Navigation
		case GLFW_KEY_UP:	 C.e.y += 0.1f;	C.a.y += 0.1f; break;
		case GLFW_KEY_DOWN:	 C.e.y -= 0.1f;	C.a.y -= 0.1f; break;
		case GLFW_KEY_LEFT:		cout << "left turn" << endl;	
			theta += 10;
			break;
		case GLFW_KEY_RIGHT:	cout << "right turn" << endl; 
			theta -= 10;
			break;
		}
	}
