...
// Compute the transparency vector of l with respect to n
vec3
transparent(const vec3& l, const vec3& n, double n1, double n2)
{
	double cos1 = dot(l, n);
	double cos2 = sqrt(1 - pow(n1 / n2, 2) * (1 - pow(cos1, 2)));
	return float(cos2 - float((n1 / n2) * cos1)) * n - float(n1 / n2) * l;
}
...
float
findInnerIntersection(const Ray& ray, const vec3& center, float radius, vec3& p, vec3& n)
{
	vec3	p10 = ray.p1 - ray.p0;
	vec3	p0c = ray.p0 - center;
	float	a = dot(p10, p10);
	...
	p = (1 - t) * ray.p0 + t * ray.p1;	// The inner intersection point
	n = normalize(p - center);  	// Normal at the point
...
//	Exclude an intersection with the object E where the ray start from. 196 vec3
vec3
	...
	if (iObject != -1) // Hit an object
	{
		...
		if (depth < DEPTH)
		{
			if (selection == 1 || selection == 4)
			{
				...
				vec3	I_R = intensity(reflectRay, l, nLights, depth + 1, iObject);
			...
			{
				// Transmision ray
				double n1 = 1.0; double n2 = 5.0;
				if (selection == 2)	n2 = 1.0;

				vec3	t = normalize(transparent(ray.p0 - ray.p1, n, n1, n2));
				vec3	pt = p + 1.0E10f * t;	// Point far awary from p along t
				Ray	refractRay(p, pt);
				
				if (selection > 2)
				{	// Transmision ray
					findInnerIntersection(refractRay, center_world[iObject], radius[iObject], p, n);
					t = normalize(transparent(refractRay.p0 - refractRay.p1, n, n2, n1));
					pt = p + 1.0E10f * t;	// Point far awary from p along t
					refractRay.p1 = pt;
				}

				vec3 I_T = intensity(refractRay, l, nLights, depth + 1, iObject);
				for (int i = 0; i < 3; i++)
					I[i] += 0.5 * I_T[i];
			}
		}
...
rayTracing()
{
	...
	// Modeling matrix
	{
		...
		// viewModel = viewModel * rotation matrix
...
#pragma omp master
	...
	cout << "Keyboard	input :	A for reflection" << endl;
	cout << "Keyboard	input :	S for refraction with Kt = 0.5" << endl;
	cout << "Keyboard	input :	D for relative refracive index 2/3" << endl;
	cout << "Keyboard	input :	F for reflection & refraction" << endl;
}
...
keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		case GLFW_KEY_A: selection = 1; break;
		case GLFW_KEY_S: selection = 2; break;
		case GLFW_KEY_D: selection = 3; break;
		case GLFW_KEY_F: selection = 4; break;
