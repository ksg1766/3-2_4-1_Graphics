...
void	render(GLFWwindow* window, GLuint program, GLuint vao, int numTris, Matrix4f& ModelMatrix);
...
main(int argc, char* argv[])
{
	...
	// VAO and VBO: a bunny in this case
	...
	// Initialization
	{
		...
		createShaders("sv02_Gouraud.glsl", "sf02_Gouraud.glsl", programGouraud, vsGouraud, fsGouraud);
		createShaders("sv02_Phong.glsl", "sf02_Phong.glsl", programPhong, vsPhong, fsPhong);
	...
	cout << "Keyboard Input: up/down for increase/decrease various coefficients" << endl;
	cout << "Keyboard Input: d for the diffuse coefficinet" << endl;
	cout << "Keyboard Input: s for the specular coefficietn" << endl;
	cout << "Keyboard Input: a for the ambient coefficinet" << endl;
	cout << "Keyboard Input: n for the shininess coefficient" << endl;
...
void
render(GLFWwindow * window, GLuint program,	GLuint vao, int numTris, Matrix4f& ModelMatrix)
{
	// Antialiasing	
	...
	if (light1)
	{
		Vector3f	l0 = ViewMatrix.block<3, 3>(0, 0) * light + ViewMatrix.block<3, 1>(0, 3) + Vector3f(10.0, 0.0, 0.0);
		setUniform(program, "LightPosition0", l0);
	}
	else
	{
		Vector3f	l0 = Vector3f(-1.0, -1.0, -1.0);
		setUniform(program, "LightPosition0", l0);
	}
	
	if (light2)
	{
		Vector3f	l1 = ViewMatrix.block<3, 3>(0, 0) * light + ViewMatrix.block<3, 1>(0, 3) - Vector3f(10.0, 0.0, 0.0);
		setUniform(program, "LightPosition1", l1);
	}
	else
	{
		Vector3f	l1 = Vector3f(-1.0, -1.0, -1.0);
		setUniform(program, "LightPosition1", l1);
	}
...
keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		case GLFW_KEY_1:	light1 = !light1; break;
		case GLFW_KEY_2:	light2 = !light2; break;
		}
	}
}
