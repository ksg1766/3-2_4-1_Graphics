...
main(int argc, char* argv[])
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Deal with the current frame if (elapsed > timeStep)
		if (elapsed > timeStep)
		{
...
drawShrunkenFaces()
{
	glPolygonMode(GL_FRONT, GL_FILL);
	// Material
	setupColoredMaterial(Vector3f(0.5f, 0.5f, 0.95f));
...
render(GLFWwindow * window)
{
	...
	if (eigen)
	{
		...
		// fed in the row - major order.Meanwhile, Eigen represents Tin the column - major order.
...
keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		case GLFW_KEY_1:	exercise =  !exercise; break;
		}
	}
}