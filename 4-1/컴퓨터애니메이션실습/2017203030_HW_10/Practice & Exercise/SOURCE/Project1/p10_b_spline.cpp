...
void
prepareRepeatedControlPoints(int _repetition)
{
...
{
	float	t2 = t1 * t1;	// Square of t
	float	t3 = t2 * t1;	// Cube of t
...
drawBSpline()
{
	...
	for (int i = 0; i < nControlPoints - 3; i++)
	{
		...
		if (ctrlPolygonDrawingEnabled)
		{
			glEnable(GL_LINE_STIPPLE);
			glLineStipple(1, 25);
			glBegin(GL_LINE_STRIP);
			for (int i = iSegment; i < nControlPoints - 2 * repetition + iSegment - 4; i++)
			{
				glVertex3fv(controlPoints[i].data());
			}
			glEnd();
